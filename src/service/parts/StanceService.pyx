...
cdef class StanceService():
    ...
    cdef bint execute_pool(self, int data_set_idx):
        ...
        try:
            ...
            if (data_set.org_model.can_arm_sizing and data_set.rep_model.can_arm_sizing) or self.options.arm_options.arm_check_skip_flg:
                if data_set.detail_stance_flg:
                    if result and "어깨 보정" in data_set.selected_stance_details:
                        # 肩補正
                ...
                if data_set.detail_stance_flg:
                    # 센터 Y 보정
                    if result and "센터 Y 보정" in data_set.selected_stance_details:
                        # 센터 Y 보정
                ...
                if not data_set.org_model.can_arm_sizing:
                    target_model_type = "작성 원본"

                if not data_set.rep_model.can_arm_sizing:
                    if len(target_model_type) > 0:
                        target_model_type = target_model_type + "/"

                    target_model_type = target_model_type + "변환용"

                logger.warning("No. %s의 %s 모델의 팔 구조에 사이징이 지원되지 않기 때문에, 팔 계통 처리를 건너뜁니다.", (data_set_idx + 1), target_model_type, decoration=MLogger.DECORATION_BOX)
            ...
            import traceback
            logger.error("사이징 처리가 의도하지 않은 오류로 종료되었습니다.\n\n%s", traceback.format_exc())
            raise e

    # 비틀림 분산
    cdef bint spread_twist(self, int data_set_idx, data_set):
        logger.info("비틀림 분산　【No.%s】", (data_set_idx + 1), decoration=MLogger.DECORATION_LINE)
        ...
        if self.options.now_process_ctrl and process_cnt == total_cnt:
            ...
            self.options.tree_process_dict[proccess_key]["비틀림 분산"] = True
    ...
    cdef int spread_twist_lr(self, int data_set_idx, str direction, bint dummy):
        ...
        try:
            ...
            if set(twist_target_bones).issubset(data_set.rep_model.bones):
                ...
                # 内積差分に基づきキー추가
                logger.info("%s비틀림 분산 준비 시작【No.%s】", direction, (data_set_idx + 1))
                ...
                logger.info("-- %s비틀림 분산 준비:종료【No.%s】", direction, (data_set_idx + 1))

                logger.info("%s비틀림 분산 준비:종료【No.%s】", direction, (data_set_idx + 1))
                logger.info("%s비틀림 분 산시작【No.%s】", direction, (data_set_idx + 1))
                ...
                logger.info("%s비틀림 분산 후처리 - 분산 중간 체크①【No.%s】", arm_bone_name, (data_set_idx + 1))
                ...
                # logger.info("%s비틀림 분산後처리 - 분산中間체크②【No.%s】", arm_bone_name, (data_set_idx + 1))
                ...
                #     futures = []
                #     with ThreadPoolExecutor(thread_name_prefix="twist_exec{0}".format(data_set_idx), max_workers=min(5, self.options.max_workers)) as executor:
                #         # 最初のfnoリストに含まれず, 체크後のfnoリストに含まれており, かつ新fnoがまだ추가されていない場合のみ체크
                ...
                # logger.info("%s비틀림 분산後처리 - 円滑化【No.%s】", arm_bone_name, (data_set_idx + 1))
                ...
                # logger.info("%s비틀림 분산後처리 - フィルタリング【No.%s】", arm_bone_name, (data_set_idx + 1))
                ...
                logger.info("%s비틀림 분산:종료【No.%s】", direction, (data_set_idx + 1))
                return PROCESS_FINISH

            else:
                logger.info("%s비틀림 분산: 【No.%s】[%s]의 본군이 작성 원본 혹은 변환용 모델중 하나라도 부족하기 때문에 처리를 건너뜁니다.", direction, (data_set_idx + 1), ", ".join(twist_target_bones))
            ...
            if is_target_copy:
                ...
                for fno in fnos:
                    ...
                    if not bf.read and bf.rotation.toDegree() < 3:
                        # 読み込みキーではなく, かつ각도が非常に小さい場合, 親ボーンの보간 곡선をコピーする
                    ...
                    if fno // 2000 > prev_sep_fno:
                        logger.count("【No.{0} - 보간 곡선 설정 - {1}】".format(data_set_idx + 1, bone_name), fno, fnos)
                        prev_sep_fno = fno // 2000
    ...
    cdef bint check_twist_pool(self, int data_set_idx, VmdMotion prev_twist_motion, int fno_idx, int fno, int last_fno, str arm_bone_name, str arm_twist_bone_name, str elbow_bone_name, \
        ...
        try:
            ...
            if 0.95 > twist_test_dot:
                # 離れていたらやり直し
                logger.debug("× 중간 괴리%s f: %s, %s, twist_test_dot: %s, twist_test_x_dot: %s, twist_test_y_dot: %s", count, fno, arm_twist_bone_name, twist_test_dot, twist_test_x_dot, twist_test_y_dot)
                ...
                logger.debug("○ 중간 일치 f: %s, %s, twist_test_dot: %s, twist_test_x_dot: %s, twist_test_y_dot: %s", fno, arm_twist_bone_name, twist_test_dot, twist_test_x_dot, twist_test_y_dot)

            if fno in log_target_idxs:
                logger.count("【No.{0} - 중간 비틀림 분산{1} - {2}】".format(data_set_idx + 1, count, arm_twist_bone_name), fno, None, last_fno=last_fno)
    ...
    cdef bint spread_twist_pool(self, int data_set_idx, int fno_idx, int fno, int last_fno, str arm_bone_name, str arm_twist_bone_name, str elbow_bone_name, \
        ...
        try:
            ...
            if fno in log_target_idxs and last_fno > 0:
                logger.count("【No.{0} - 비틀림 분산 - {1}】".format(data_set_idx + 1, arm_twist_bone_name), fno, None, last_fno=last_fno)
    ...
    cdef tuple calc_arm_twist_elbow_qq(self, int data_set_idx, int fno, str arm_bone_name, MVector3D arm_local_x_axis, MVector3D arm_local_y_axis, MQuaternion original_arm_qq, \
        ...
        while i < 5:
            ...
            if i == 0:
                # 初回は初期化
                logger.debug("(%s) 팔 비틀림 초기화 f: %s, %s: arm_twist_degree: %s", i, fno, elbow_bone_name, arm_twist_degree)
            ...
            # ひじZに回っている場合, ローカルY軸の체크を変える
            if is_elbow_z:
                # separate_elbow_mat.rotate(elbow_y2z_qq)
                if elbow_degree < 0:
                    logger.debug("(%s) Z 역 팔꿈치 f: %s, %s: yz: %s, y: %s, z: %s", i, fno, elbow_bone_name, elbow_degree, elbow_y_qq.toDegree(), elbow_z_qq.toDegree())
                else:
                    logger.debug("(%s) Z 정 팔꿈치 f: %s, %s: yz: %s, y: %s, z: %s", i, fno, elbow_bone_name, elbow_degree, elbow_y_qq.toDegree(), elbow_z_qq.toDegree())
            else:
                if elbow_degree < 0:
                    logger.debug("(%s) Y 역 팔꿈치 f: %s, %s: yz: %s, y: %s, z: %s", i, fno, elbow_bone_name, elbow_degree, elbow_y_qq.toDegree(), elbow_z_qq.toDegree())
                else:
                    logger.debug("(%s) Y 정 팔꿈치 f: %s, %s: yz: %s, y: %s, z: %s", i, fno, elbow_bone_name, elbow_degree, elbow_y_qq.toDegree(), elbow_z_qq.toDegree())
            ...
            for j, elbow_degree in enumerate(elbow_degree_list):
                ...
                if elbow_result_dot < prev_elbow_result_dot and twist_test_y_dot <= 0 and i > 0:
                    # ひじが近付いてない場合, この時点でNG
                    logger.debug("× 팔꿈치 기각 (%s-%s) f: %s, %s, x_weight: %s, elbow_result_dot: %s, prev_elbow_result_dot: %s", i, j, fno, elbow_bone_name, x_weight, elbow_result_dot, prev_elbow_result_dot)
                ...
                logger.debug("임시 설정(%s-%s) f: %s, %s, x_weight: %s, elbow_result_dot: %s, elbow_result_degree: %s", i, j, fno, elbow_bone_name, x_weight, elbow_result_dot, elbow_result_degree)
                ...
                logger.debug("후보(%s-%s) f: %s, %s, degree_list: %s, elbow: %s, x_weight: %s", i, j, fno, arm_twist_bone_name, degree_list, elbow_result_degree, x_weight)
                ...
                arm_twist_result_qq = MQuaternion.fromAxisAndAngle(arm_twist_local_x_axis, arm_twist_result_degree)
                logger.debug("仮설정(%s-%s) f: %s, %s, arm_twist_result_dot: %s, arm_twist_result_degree: %s", i, j, fno, arm_twist_bone_name, arm_twist_result_dot, arm_twist_result_degree)
                ...
                if prev_arm_twist_result_dot > arm_twist_result_dot or (np.sign(elbow_result_degree) != np.sign(prev_elbow_result_degree) and prev_elbow_result_dot > elbow_result_dot):
                    logger.debug("(%s-%s) f: %s, %s, 전회 재임용", i, j, fno, arm_twist_bone_name)
                    # ダメだった場合, 前回のを採用し直し
                    arm_twist_result_dot = prev_arm_twist_result_dot
                    arm_twist_result_degree = prev_arm_twist_result_degree
                    elbow_result_dot = prev_elbow_result_dot
        ...
        if arm_twist_result_dot < 0.9:
            logger.warning("【No.%s】%s프레임째:%s 비틀림 분산 실패: 각도: %s 근사도: %s", (data_set_idx + 1), fno, arm_twist_bone_name, round(arm_twist_result_degree, 3), round(arm_twist_result_dot, 5))
    ...
    cdef tuple calc_wrist_twist_elbow_qq(self, int data_set_idx, int fno, str arm_bone_name, MVector3D arm_local_x_axis, MVector3D arm_local_y_axis, MQuaternion original_arm_qq, MQuaternion arm_qq, \
        ...
        while i < 5:
            ...
            for j, wrist_result_qq in enumerate(wrist_qq_list):
                ...
                logger.debug("후보(%s-%s) f: %s, %s, degree_list: %s, wrist: %s", i, j, fno, wrist_twist_bone_name, degree_list, wrist_result_qq.toDegree())
                ...
                wrist_twist_result_qq = MQuaternion.fromAxisAndAngle(wrist_twist_local_x_axis, wrist_twist_result_degree)
                logger.debug("임시 설정(%s-%s) f: %s, %s, wrist_twist_result_dot: %s, wrist_twist_result_degree: %s", i, j, fno, wrist_twist_bone_name, wrist_twist_result_dot, wrist_twist_result_degree)
                ...
                if wrist_result_dot < prev_wrist_result_dot and i > 0:
                    # 手首が近付いてない場合, この時点でNG
                    logger.debug("× 손목 기각 (%s-%s) f: %s, %s, wrist_result_dot: %s, prev_wrist_result_dot: %s", i, j, fno, wrist_bone_name, wrist_result_dot, prev_wrist_result_dot)
                    wrist_result_dot = prev_wrist_result_dot
                    wrist_result_qq = prev_wrist_result_qq

                logger.debug("임시 거치(%s-%s) f: %s, %s, wrist_result_dot: %s, wrist_result_qq: %s", i, j, fno, wrist_bone_name, wrist_result_dot, wrist_result_qq)
        ...
        if wrist_twist_result_dot < 0.9:
            logger.warning("【No.%s】%s프레임째:%s 비틀림 분산 실패: 각도: %s 근사도: %s", (data_set_idx + 1), fno, wrist_twist_bone_name, round(wrist_twist_result_degree, 3), round(wrist_twist_result_dot, 5))
    ...
    cdef bint adjust_leg_ik_stance(self, int data_set_idx, MOptionsDataSet data_set):
        logger.info("발 IK 보정　【No.%s】", (data_set_idx + 1), decoration=MLogger.DECORATION_LINE)
        ...
        if self.options.now_process_ctrl and process_cnt == total_cnt:
            ...
            proccess_key = "【No.{0}】{1}({2})".format(data_set_idx + 1, os.path.basename(data_set.motion.path), data_set.rep_model.name)
            self.options.tree_process_dict[proccess_key]["자세 추가 보정"]["발 IK 보정"] = True
        ...
        try:
            ...
            if set(leg_ik_target_bones).issubset(data_set.org_model.bones) and set(leg_ik_target_bones).issubset(data_set.rep_model.bones):
                ...
                if data_set.motion.is_active_bones("{0}足IK親".format(direction)) and ("{0}足IK親".format(direction) not in data_set.org_model.bones or "{0}足IK親".format(direction) not in data_set.rep_model.bones):
                    logger.info("%s발 IK 보정: [No.%s] %s 발 IK 부모가 작성 원본 혹은 변환용 모델 둘 중 하나에 없기 때문에 처리를 건너뜁니다.", direction, (data_set_idx + 1), direction)
                    return PROCESS_SKIP

                # 足ＩＫのそれぞれでフレーム番号を체크する
                prev_sep_fno = 0
                if target_bone_name in data_set.motion.bones and \
                    ...
                    logger.info("%s보정【No.%s】", target_bone_name, (data_set_idx + 1))
                    ...
                    if len(fnos) == 0:
                        logger.info("%s발 IK 보정: 【No.%s】처리 대상 키 프레임이 없어서, 처리를 종료합니다.", direction, (data_set_idx + 1))
                        return True
                    ...
                    for fno_idx, fno in enumerate(fnos):
                        if fno not in ik_on_fnos:
                            # IK=ONの키 프레임ではない場合, 처리スルー
                            logger.warning("【No.%s】%s프레임째:%s IK플래그=OFF이므로, 처리를 건너뜁니다.", (data_set_idx + 1), fno, target_bone_name)
                            continue

                        if fno in d_on_fnos:
                            # D系ボーンに値が入ってる키 프레임である場合, 처리スルー
                            logger.warning("【No.%s】%s프레임째:%s 다리D 혹은 무릎D에 값이 들어가 있기 때문에, 처리 스킵합니다.", (data_set_idx + 1), fno, target_bone_name)
                            continue
                        ...
                        logger.debug("f: %s, %s, 선 IK 로컬(계산 전): %s, 선 IK 로컬(계산 후): %s, 변경 후IK: %s", fno, target_bone_name, \
                        ...
                        if fno // 500 > prev_sep_fno:
                            logger.count("【No.{0} - {1}보정】".format(data_set_idx + 1, target_bone_name), fno, fnos)
                            prev_sep_fno = fno // 500

                    logger.info("%s발 IK 보정:종료【No.%s】", direction, (data_set_idx + 1))
                    return PROCESS_FINISH

                else:
                    logger.info("%s발 IK 보정: 【No.%s】작성 원본 혹은 변환용의 %s발 IK의 IK 루트본이 %s발 본이 아니기 때문에 처리를 건너뜁니다.", direction, (data_set_idx + 1), direction, direction)
            else:
                logger.info("%s발 IK 보정: 【No.%s】[%s]의 본군이 작성 원본 혹은 변환용 중 하나라도 부족하기 때문에 처리를 건너뜁니다.", direction, (data_set_idx + 1), ", ".join(leg_ik_target_bones))
    ...
    cdef bint adjust_toe_ik_stance(self, int data_set_idx, MOptionsDataSet data_set):
        logger.info("발끝 IK 보정　【No.%s】", (data_set_idx + 1), decoration=MLogger.DECORATION_LINE)
        ...
        if self.options.now_process_ctrl and process_cnt == total_cnt:
            ...
            proccess_key = "【No.{0}】{1}({2})".format(data_set_idx + 1, os.path.basename(data_set.motion.path), data_set.rep_model.name)
            self.options.tree_process_dict[proccess_key]["자세 추가 보정"]["발끝 IK 보정"] = True
            ...
            if set(toe_ik_target_bones).issubset(data_set.org_model.bones) and set(toe_ik_target_bones).issubset(data_set.rep_model.bones):
                ...
                if toe_ik_bone_name in data_set.org_model.bones and toe_ik_bone_name in data_set.rep_model.bones \
                    ...
                    if not data_set.motion.is_active_bones(toe_ik_bone_name):
                        # 0Fキーはあっても無視
                        logger.info("%s발끝 IK 보정: 【No.%s】처리대상 키 프레임이 없기 때문에 처리를 종료합니다.", direction, (data_set_idx + 1))
                        return PROCESS_SKIP

                    logger.info("%s보정【No.%s】", toe_ik_bone_name, (data_set_idx + 1))
                    ...
                    for fno_idx, fno in enumerate(fnos):
                        if fno not in ik_on_fnos:
                            # IK=ONの키 프레임ではない場合, 처리スルー
                            logger.warning("【No.%s】%s프레임째:%s IK플래그=OFF이기 때문에 처리를 건너뜁니다.", (data_set_idx + 1), fno, toe_ik_bone_name)
                            continue

                        if fno in d_on_fnos:
                            # D系ボーンに値が入ってる키 프레임である場合, 처리スルー
                            logger.warning("【No.%s】%s프레임째:%s 다리 D 혹은 무릎 D에 값이 들어가 있기 때문에, 처리를 건너뜁니다.", (data_set_idx + 1), fno, toe_ik_bone_name)
                            continue
                ...
                logger.info("%s발끝 IK 보정:종료【No.%s】", direction, (data_set_idx + 1))
                return PROCESS_FINISH
            else:
                logger.info("%s발끝 IK 보정: 【No.%s】[%s]의 본군이 작성 원본 또는 변환용 모델중 하나에 부족하기 때문에,  처리를 건너뜁니다.", direction, (data_set_idx + 1), ", ".join(toe_ik_target_bones))
    ...
    cdef bint adjust_toe_stance(self, int data_set_idx, MOptionsDataSet data_set):
        logger.info("발끝 보정　【No.%s】", (data_set_idx + 1), decoration=MLogger.DECORATION_LINE)
        ...
        if self.options.now_process_ctrl and process_cnt == total_cnt:
            ...
            proccess_key = "【No.{0}】{1}({2})".format(data_set_idx + 1, os.path.basename(data_set.motion.path), data_set.rep_model.name)
            self.options.tree_process_dict[proccess_key]["자세 추가 보정"]["발끝 보정"] = True
    ...
    cdef int adjust_toe_stance_lr(self, int data_set_idx, str direction, str dummy):
        ...
        try:
            ...
            # 足IK親がモーションにあって, かつモデルにない場合, 元の位置がおかしいのでスキップ
            if data_set.motion.is_active_bones("{0}足IK親".format(direction)) and ("{0}足IK親".format(direction) not in data_set.org_model.bones or "{0}足IK親".format(direction) not in data_set.rep_model.bones):
                logger.info("%s발끝 보정: [No.%s] %s 다리 IK 부모가 작성 원본 혹은 변환용 모델 중 하나에 부족하기 때문 에처리를 건너뜁니다.", direction, (data_set_idx + 1), direction)
                return PROCESS_SKIP
            ...
            if set(toe_target_bones).issubset(data_set.org_model.bones) and set(toe_target_bones).issubset(data_set.rep_model.bones):
                ...
                if direction == "左":
                    logger.debug("원본：왼쪽 발끝：%s", data_set.org_model.left_toe_vertex)
                    logger.debug("변환용：왼쪽 발끝：%s", data_set.rep_model.left_toe_vertex)
                    logger.debug("원본：왼쪽 발바닥：%s", data_set.org_model.left_sole_vertex)
                    logger.debug("변환용：왼쪽 발바닥：%s", data_set.rep_model.left_sole_vertex)
                else:
                    logger.debug("원본：오른쪽 발끝：%s", data_set.org_model.right_toe_vertex)
                    logger.debug("변환용：오른쪽 발끝：%s", data_set.rep_model.right_toe_vertex)
                    logger.debug("원본：오른쪽 발바닥：%s", data_set.org_model.right_sole_vertex)
                    logger.debug("변환용：오른쪽 발바닥：%s", data_set.rep_model.right_sole_vertex)
                ...
                logger.info("%s발끝 보정【No.%s】", direction, (data_set_idx + 1))

                prev_sep_fno = 0
                # 足ＩＫと足IK親の両方でフレーム番号を체크する
                ik_bone_name = "{0}足ＩＫ".format(direction)
                fnos = data_set.motion.get_bone_fnos(ik_bone_name, "{0}足IK親".format(direction))
                for fno_idx, fno in enumerate(fnos):
                    # 足ＩＫのbf(この時点では登録するか分からないので, 보간 곡선リセットなし)
                    ...
                    if rep_toe_pos.y() <= rep_sole_pos.y() and org_toe_pos.y() < org_toe_limit:
                        ...
                        logger.debug("f: %s, %s 발끝 바닥 보정: 발끝 맞추기 발끝 실체:%s, 발바닥 실체:%s, 발IK:%s", ik_bf.fno, direction, rep_toe_pos.y(), rep_sole_pos.y(), adjust_toe_y)
                        # 登録대상
                        ...
                        logger.debug("f: %s, %s 발바닥 보정: 발바닥 맞춤 발바닥 실체:%s,발바닥 실체:%s,발IK:%s", ik_bf.fno, direction, rep_sole_pos.y(), rep_sole_pos.y(), adjust_sole_y)
                        # 登録대상
                        data_set.motion.regist_bf(ik_bf, "{0}足ＩＫ".format(direction), fno)
                    else:
                        # 발끝 보정 없이
                    ...
                    if fno // 500 > prev_sep_fno:
                        logger.count("【No.{0} - {1}발끝 보정】".format(data_set_idx + 1, direction), fno, fnos)
                        prev_sep_fno = fno // 500

                logger.info("%s발끝 보정:종료【No.%s】", direction, (data_set_idx + 1))
                return PROCESS_FINISH
            else:
                logger.info("%s발끝 보정: 【No.%s】[%s]의 본군이 작성 원본 또는 변환용 모델 중 하나에 부족하기 때문에,  처리를 건너뜁니다.", direction, (data_set_idx + 1), ", ".join(toe_target_bones))
    ...
    cdef bint adjust_center_stance(self, int data_set_idx, MOptionsDataSet data_set):
        ...
        logger.info("센터 XZ 보정　【No.%s】", (data_set_idx + 1), decoration=MLogger.DECORATION_LINE)
        ...
        if set(center_target_bones).issubset(data_set.org_model.bones) and set(center_target_bones).issubset(data_set.rep_model.bones) and "センター" in data_set.motion.bones:
            # 判定用のセンターボーン名(グルーブがある場合, グルーブまでを대상とする)
            ...
            logger.info("센터 XZ 보정: 준비종료【No.%s】", (data_set_idx + 1))
            ...
            for fno in fnos:
                ...
                if bf.key:
                    logger.debug("f: %s, 조정 전: %s", bf.fno, bf.position)
                    ...
                    logger.debug("f: %s, 발 IK 오프셋 후: %s", bf.fno, bf.position)
                    bf.position += self.calc_center_offset_by_trunk(bf, data_set_idx, data_set, \
                                                                    ...
                                                                    org_center_bone_name, rep_center_bone_name)
                    logger.debug("f: %s, 체간 오프셋 후: %s", bf.fno, bf.position)

                if fno // 500 > prev_fno:
                    logger.count("【No.{0} - 센터 XZ 보정】".format(data_set_idx + 1), fno, fnos)
                    prev_fno = fno // 500

            logger.info("센터 XZ 보정: 종료【No.%s】", (data_set_idx + 1))
            ...
            if self.options.now_process_ctrl:
                ...
                proccess_key = "【No.{0}】{1}({2})".format(data_set_idx + 1, os.path.basename(data_set.motion.path), data_set.rep_model.name)
                self.options.tree_process_dict[proccess_key]["자세 추가 보정"]["센터 XZ 보정"] = True

        else:
            logger.info("센터 XZ 보정: 【No.%s】[%s]의 본군이 작성 원본 또는 변환용 모델 중 하나에 부족하기 때문에, 처리를 건너뜁니다.", (data_set_idx + 1), ", ".join(center_target_bones))
    ...
    cdef bint adjust_center_arm_stance(self, int data_set_idx, MOptionsDataSet data_set):
        ...
        logger.info("센터 Y 보정　【No.%s】", (data_set_idx + 1), decoration=MLogger.DECORATION_LINE)

        # センター調整に必要なボーン群(腕체크済み)
        ...
        if set(center_target_bones).issubset(data_set.org_model.bones) and set(center_target_bones).issubset(data_set.rep_model.bones) and "センター" in data_set.motion.bones:
            ...
            logger.info("센터 Y 보정: 준비종료【No.%s】", (data_set_idx + 1))
            ...
            for fno in fnos:
                ...
                if bf.position.y() < 0:
                    logger.debug("f: %s, 조정 전: %s", target_bf.fno, target_bf.position)
                    ...
                    logger.debug("f: %s, 팔 오프셋 후: %s", target_bf.fno, target_bf.position)
                ...
                if fno // 500 > prev_fno and fnos[-1] > 0:
                    logger.count("【No.{0} - 센터 Y 보정】".format(data_set_idx + 1), fno, fnos)
                    prev_fno = fno // 500

            logger.info("센터 Y 보정: 종료【No.%s】", (data_set_idx + 1))
            ...
            if self.options.now_process_ctrl:
                ...
                proccess_key = "【No.{0}】{1}({2})".format(data_set_idx + 1, os.path.basename(data_set.motion.path), data_set.rep_model.name)
                self.options.tree_process_dict[proccess_key]["자세 추가 보정"]["센터 Y 보정"] = True

        else:
            logger.info("센터 Y 보정: 【No.%s】[%s]의 본군이 작성 원본 또는 변환용 모델 중 하나에 부족하기 때문에, 처리를 건너뜁니다.", (data_set_idx + 1), ", ".join(center_target_bones))
    ...
    cdef tuple calc_center_offset_by_arm(self, VmdBoneFrame bf, int data_set_idx, MOptionsDataSet data_set, \
        ...
        if is_org_left_wrist_offset:
            ...
            logger.debug("○ 센터 Y 보정(왼쪽 손목) f: %s, y: %s, org: %s, rep: %s", bf.fno, \
                         org_left_wrist_offset, (org_left_wrist_pos.y()), rep_left_wrist_pos.y())
        else:
            logger.debug("× 센터 Y 보정 없이(왼쪽 손목) f: %s, org: %s", bf.fno, org_left_wrist_pos.y())
        ...
        if is_org_right_wrist_offset:
            ...
            logger.debug("○ 센터 Y 보정(오른쪽 손목) f: %s, y: %s, org: %s, rep: %s", bf.fno, \
                         org_right_wrist_offset, (org_right_wrist_pos.y()), rep_right_wrist_pos.y())
        else:
            logger.debug("× 센터 Y 보정 없이(오른쪽 손목) f: %s, org: %s", bf.fno, org_left_wrist_pos.y())
        ...
        if is_org_left_leg_offset:
            ...
            logger.debug("○ 센터 Y 보정(왼쪽 다리) f: %s, y: %s, org: %s, rep: %s", bf.fno, \
                         org_left_leg_offset, (org_left_leg_pos.y()), rep_left_leg_pos.y())
        else:
            logger.debug("× 센터 Y 보정 없이(왼쪽 다리) f: %s, org: %s", bf.fno, org_left_wrist_pos.y())
        ...
        if is_org_right_leg_offset:
            ...
            logger.debug("○ 센터 Y 보정(오른쪽 다리) f: %s, y: %s, org: %s, rep: %s", bf.fno, \
                         org_right_leg_offset, (org_right_leg_pos.y()), rep_right_leg_pos.y())
        else:
            logger.debug("× 센터 Y 보정 없이(오른쪽 다리) f: %s, org: %s", bf.fno, org_left_wrist_pos.y())
        ...
        logger.debug("센터 Y 보정(결과) f: %s, y: %s", bf.fno, rep_center_arm_offset.y())
    ...
    cdef bint adjust_upper_stance(self, int data_set_idx, MOptionsDataSet data_set):
        ...
        logger.info("상반신 보정　【No.%s】", (data_set_idx + 1), decoration=MLogger.DECORATION_LINE)
        ...
        if set(upper_target_bones).issubset(data_set.org_model.bones) and set(upper_target_bones).issubset(data_set.rep_model.bones) and "上半身" in data_set.motion.bones:
            ...
            logger.info("【No.%s】상반신 - 방향의 근사도: %s", (data_set_idx + 1), round(dot, 5))
            ...
            logger.info("상반신 보정: 준비종료【No.%s】", (data_set_idx + 1))
            ...
            for fno_idx, fno in enumerate(fnos):
                ...
                if fno // 500 > prev_fno:
                    logger.count("【No.{0} - 상반신 보정】".format(data_set_idx + 1), fno, fnos)
                    prev_fno = fno // 500
            ...
            logger.info("상반신 보정: 종료【No.%s】", (data_set_idx + 1))
            ...
            if is_upper2_existed:
                ...
                logger.info("【No.%s】상반신2 - 방향의 근사도: %s", (data_set_idx + 1), round(dot, 5))
                ...
                logger.info("상반신2 보정: 준비종료【No.%s】", (data_set_idx + 1))
                ...
                for fno_idx, fno in enumerate(fnos):
                    ...
                    if fno // 500 > prev_fno:
                        logger.count("【No.{0} - 상반신2 보정】".format(data_set_idx + 1), fno, fnos)
                        prev_fno = fno // 500
                ...
                logger.info("상반신2 보정: 종료【No.%s】", (data_set_idx + 1))
            ...
            if self.options.now_process_ctrl:
                ...
                proccess_key = "【No.{0}】{1}({2})".format(data_set_idx + 1, os.path.basename(data_set.motion.path), data_set.rep_model.name)
                self.options.tree_process_dict[proccess_key]["자세 추가 보정"]["상반신 보정"] = True

        else:
            logger.info("상반신 보정: 【No.%s】[%s]의 본군이 작성 원본 또는 변환용 모델 중 하나에 부족하기 때문에, 처리를 건너뜁니다.", (data_set_idx + 1), ", ".join(upper_target_bones))
    ...
    cdef bint adjust_lower_stance(self, int data_set_idx, MOptionsDataSet data_set):
        ...
        logger.info("하반신 보정　【No.%s】", (data_set_idx + 1), decoration=MLogger.DECORATION_LINE)
        ...
        if set(lower_target_bones).issubset(data_set.org_model.bones) and set(lower_target_bones).issubset(data_set.rep_model.bones) and "下半身" in data_set.motion.bones:
            ...
            logger.info("【No.%s】하반신 - 방향의 근사도: %s", (data_set_idx + 1), round(dot, 5))
            ...
            logger.info("하반신 보정: 준비종료【No.%s】", (data_set_idx + 1))
            ...
            for fno_idx, fno in enumerate(fnos):
                ...
                if fno // 500 > prev_fno:
                    logger.count("【No.{0} - 하반신 보정】".format(data_set_idx + 1), fno, fnos)
                    prev_fno = fno // 500
            ...
            logger.info("하반신 보정: 종료【No.%s】", (data_set_idx + 1))
            ...
            if self.options.now_process_ctrl:
                ...
                proccess_key = "【No.{0}】{1}({2})".format(data_set_idx + 1, os.path.basename(data_set.motion.path), data_set.rep_model.name)
                self.options.tree_process_dict[proccess_key]["자세 추가 보정"]["하반신 보정"] = True
        else:
            logger.info("하반신 보정: 【No.%s】[%s]의 본군이 작성 원본 또는 변환용 모델 중 하나에 부족하기 때문에, 처리를 건너뜁니다.", (data_set_idx + 1), ", ".join(lower_target_bones))
    ...
    cdef bint adjust_shoulder_stance(self, int data_set_idx, MOptionsDataSet data_set):
        ...
        if self.options.now_process_ctrl and process_cnt == total_cnt:
            ...
            proccess_key = "【No.{0}】{1}({2})".format(data_set_idx + 1, os.path.basename(data_set.motion.path), data_set.rep_model.name)
            self.options.tree_process_dict[proccess_key]["자세 추가 보정"]["어깨 보정"] = True
    ...
    cdef int adjust_shoulder_stance_lr(self, int data_set_idx, str shoulder_p_name, str shoulder_name, str arm_name):
        ...
        try:
            ...
            if set(shoulder_target_bones).issubset(data_set.org_model.bones) and set(shoulder_target_bones).issubset(data_set.rep_model.bones) and shoulder_name in data_set.motion.bones:
                ...
                logger.info("%s자세 보정: 준비종료【No.%s】", shoulder_name, (data_set_idx + 1))
                ...
                for fno_idx, fno in enumerate(fnos):
                    ...
                    logger.debug("f: %s, %s, 보정 회전: %s, 원래의 회전: %s", bf.fno, bf.name, new_shoulder_qq.toEulerAngles4MMD().to_log(), bf.rotation.toEulerAngles4MMD().to_log())
                    ...
                    if fno // 500 > prev_fno:
                        logger.count("【No.{0} - {1}자세 보정】".format(data_set_idx + 1, shoulder_name), fno, fnos)
                        prev_fno = fno // 500

                # 子の각도調整
                self.adjust_rotation_by_parent(data_set_idx, data_set, arm_name, shoulder_name)

                logger.info("%s자세 보정: 종료【No.%s】", shoulder_name, (data_set_idx + 1))

                return PROCESS_FINISH
            else:
                logger.info("%s補正: 【No.%s】[%s]의 본군이 작성 원본 또는 변환용 모델 중 하나에 부족하기 때문에, 처리를 건너뜁니다.", shoulder_name, (data_set_idx + 1), ", ".join(shoulder_target_bones))
    ...
    cdef int adjust_shoulder_stance_lr2(self, int data_set_idx, str shoulder_p_name, str shoulder_name, str arm_name):
        ...
        try:
            ...
            if set(shoulder_target_bones).issubset(data_set.org_model.bones) and set(shoulder_target_bones).issubset(data_set.rep_model.bones) and shoulder_name in data_set.motion.bones:
                ...
                logger.info("【No.%s】%s - 방향의 근사도: %s", (data_set_idx + 1), shoulder_name, round(dot, 5))
                ...
                if round(org_shoulder_diff.y(), 1) == 0:
                    # ゼロに近い場合, 分母なので比率が馬鹿でかくなるため, 強制的に1を설정する
                ...
                logger.info("【No.%s】%s - 길이 비율: %s", (data_set_idx + 1), shoulder_name, shoulder_diff_ratio.to_log())
                ...
                if dot >= 0.82:
                    ...
                    logger.warning("%s의 초기 자세의 각도가 크게 다르기 때문에 어깨 보정 결과가 이상해질 수 있습니다.【No.%s】", shoulder_name, (data_set_idx + 1))
    ...
    cdef bint adjust_shoulder_stance_far(self, int data_set_idx, str shoulder_p_name, str shoulder_name, str arm_name, double dot_limit, bint is_shoulder_p):
        ...
        # TOの길이 비율(いかり肩ボーンとかあるので, 絶対値はとらない)
    ...
    cdef bint adjust_arm_stance(self, int data_set_idx, MOptionsDataSet data_set):
        ...
        logger.info("팔 자세 보정　【No.%s】", (data_set_idx + 1), decoration=MLogger.DECORATION_LINE)
        ...
        if self.options.now_process_ctrl:
            ...
            proccess_key = "【No.{0}】{1}({2})".format(data_set_idx + 1, os.path.basename(data_set.motion.path), data_set.rep_model.name)
            self.options.tree_process_dict[proccess_key]["팔 자세 보정"] = True
    ...
    cdef bint adjust_arm_stance_twist_pool(self, int data_set_idx, str bone_name):
        ...
        try:
            ...
            if bone_name in data_set.motion.bones and bone_name in data_set.org_model.bones and bone_name in data_set.rep_model.bones:
                ...
                logger.info("팔 자세 보정【No.%s - %s】", (data_set_idx + 1), bone_name)
                        ...
                        if org_wrist2finger1 > org_finget12finger3 * 2 or rep_wrist2finger1 > rep_finget12finger3 * 2:
                            ...
                            if direction == "左":
                                if org_wrist2finger1 > org_finget12finger3 * 2:
                                    logger.warning(f"작성 원본 모델의, 손목부터 가운데 손가락 끝까지의 길이({round(org_wrist2finger1, 4)})가운뎃손가락 길이 x 2 ({round(org_finget12finger3 * 2, 4)})보다 길기 때문에 손목의 표시처에서 기울기를 조정합니다.【No.{(data_set_idx + 1)}】")

                                if rep_wrist2finger1 > rep_finget12finger3 * 2:
                                    logger.warning(f"변환용 모델의, 손목부터 가운데 손가락 끝까지의 길이({round(rep_wrist2finger1, 4)})가운뎃손가락 길이 x 2 ({round(rep_finget12finger3 * 2, 4)})보다 길기 때문에 손목의 표시처에서 기울기를 조정합니다.【No.{(data_set_idx + 1)}】")
