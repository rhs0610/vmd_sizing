...
class CameraService():
    ...
    def execute(self):
        logger.info("카메라 보정　", decoration=MLogger.DECORATION_LINE)
        ...
        try:
            ...
            if len(self.target_data_set_idxs) == 0:
                # データセットがない場合, 処理スキップ
                logger.warning("카메라 보정이 가능한 파일 세트를 찾을 수 없었기 때문에, 카메라 보정을 스킵합니다.", decoration=MLogger.DECORATION_BOX)
                return True
            ...
            for fno in sorted(self.options.camera_motion.cameras.keys()):
                ...
                if prev_fno >= 0:
                    # 前回と同じカメラ위치の場合, 前回の사이징済みカメラ위치コピー
                    ...
                    if past_cf.org_length == cf.length and past_cf.org_position == cf.position and past_cf.euler == cf.euler:
                        logger.info("%s프레임째 전위치 및 거리 복사", fno)
            ...
            if self.options.now_process_ctrl:
                ...
                self.options.tree_process_dict["카메라 보정"] = True
    ...
    def execute_rep_camera(self, fno: int, cf: VmdCameraFrame, past_cf: VmdCameraFrame, org_inner_global_poses: dict, org_inner_square_poses: dict, rep_inner_global_poses: dict, \
        ...
        # 화면内に映ってるボーンINDEXの中央値を仮の中央座標とする
        ...
        # 거리を加味したカメラの原点（거리0でこの위치に合わせると注視点が合う）
        ...
        if camera_length < 5:
            ...
            # 原点用比率は, 거리が可動範囲内ならば比率そのまま, 範囲外ならば体格比率まで
        ...
        # この時点の거리と위치で변환용 모델の体幹＋目の映り具合をチェック（上下が分かってないと크기取れない）
        # 거리制限が掛かっている場合, スルー
        ...
        logger.info("%s프레임째 원점 비율: %s, 거리 비율: %s, 주시점: %s-%s, 윗부분: %s-%s, 아랫부분: %s-%s, 거리 오프셋: %s, 시야각 오프셋: %s", \
    ...
    def calc_camera_ratio(self, fno: int, cf: VmdCameraFrame):
        ...
        # まず全体のグローバル위치とプロジェクション座標正規위치を算出
        ...
        # 화면内に映っている顔系INDEX(注視点直近算出のためなので, ちょっと範囲広め)
        ...
        if len(org_face_inner_square_poses.keys()) > 0:
            # 顔系が화면内に映っている場合, その時点で注視点直近を取得する
            ...
            if len(org_trunk_inner_square_poses.keys()) > 0:
                # 体幹系が화면内に映っている場合, その時点で注視点直近を取得する
                ...
                # 体幹が화면に映ってない場合, 末端チェック

                # 体幹で最も注視点に近いINDEXを取得する（화면内とは限らない）
    ...
    # 指定されたボーンのうち, 大体화면内に映っているINDEXを返す
    ...
    def calc_top_botom_index(self, fno: int, cf: VmdCameraFrame, all_square_poses: dict, org_inner_global_poses: dict, x_offset: float, y_offset: float):
        ...
        for yi in y_indexes:
            if square_poses[yi][1] > square_poses[refrected_indexes[0][y_indexes[0]]][1] + 0.1:
                # 上端から화면0.2の거리までなめたら終了
                logger.debug("f: %s, 화면 상단 break: yi: %s, top: %s", fno, yi, square_poses[refrected_indexes[0][y_indexes[0]]][1])
                break

            logger.debug("f: %s, 화면 check: square_poses[yi][1]: %s, square_poses[yi][2]: %s, top_edge_pos[1]: %s, top_edge_pos[2]: %s", \
        ...
        for yi in reversed(y_indexes):
            if square_poses[yi][1] < square_poses[refrected_indexes[0][y_indexes[-1]]][1] - 0.2:
                # 下端から화면0.2の거리までなめたら終了
                logger.debug("f: %s, 화면 하단 break: yi: %s, top: %s", fno, yi, square_poses[refrected_indexes[0][y_indexes[-1]]][1])
                break
    ...
    def prepare_ratio(self, data_set_idx: int, org_model: PmxModel, rep_model: PmxModel):
        ...
        logger.info("【No.%s】작성 원본 모델 길이: %s, 머리 몸: %s, 머리 크기: %s", (data_set_idx + 1), round(org_total_height, 5), round(org_heads, 5), round(org_face_length, 5))
        logger.info("【No.%s】변환용 모델 길이: %s, 머리 몸: %s, 머리 크기: %s, Y오프셋: %s", (data_set_idx + 1), round(rep_total_height, 5), round(rep_heads, 5), round(rep_face_length, 5), data_set.camera_offset_y)
    ...
    def calc_ratio(self, data_set_idx: int, model: PmxModel, model_type: str, camera_offset_y: float):
        if model.head_top_vertex.index < 0:
            logger.warning("【No.%s】%s모델의 정수리 정점 INDEXが보이지 않기 때문에, 머리 본 + 상반신 절반의 위치로 대신 사용합니다.\n" \
                           + "길이 Y 오프셋으로 정수리 위치를 조정하면, 카메라의 견절(역주: 인물이 카메라 안에 들어있지 않은 경우를 일컬음) 등이 적어집니다.", (data_set_idx + 1), model_type)
        else:
            logger.info("【No.%s】%s모델의 정수리 정점 INDEX: %s (%s)", (data_set_idx + 1), model_type, model.head_top_vertex.index, \
